...
$listCates = getRaw("SELECT *, (SELECT count(blog.id) FROM blog
WHERE blog.category_id = blog_categories.id) as blog_count
FROM blog_categories $filter ORDER BY create_at DESC LIMIT $offset, $perPage");
    ...
    <section class="content">
        <div class="container-fluid">
            ...
            <div class="row">
                        ...
                        if (!empty($listCates)):
                            ...
                            foreach ($listCates as $item):
                                ...
                                <tr>
                                    ...
                                    <td><a href="<?php echo getLinkAdmin('blog_categories', '', ['id'=>$item['id'], 'view'=>'edit']); ?>">
                                            <?php echo $item['name']; ?></a> (<?php echo $item['blog_count']; ?>)
                                        <a href="<?php echo getLinkAdmin('blog_categories', 'duplicate', ['id'=>$item['id']]); ?>" class="btn-success btn-sm" style="float: right">Nhân bản</a></td>
                                    <td><?php echo getDateFormat($item['create_at'], 'd/m/Y H:i:s'); ?></td>
                                    <td class="text-center"><a href="<?php echo getLinkAdmin('blog_categories', '' ,['id'=>$item['id'], 'view'=>'edit']); ?>"
                                                               class="btn btn-warning btn-sm"><i class="fa fa-edit"></i></a></td>
                                    <td class="text-center"><a href="<?php echo getLinkAdmin('blog_categories', '', ['id'=>$item['id'], 'view'=>'delete']); ?>"
                ...
                <a class="page-link" href="'._WEB_HOST_ROOT_ADMIN.'?module=blog_categories'.$queryString.'&page='.$prevPage.'" 
                    aria-label="Previous">
                    <span aria-hidden="true">&laquo;</span>
                            ...
                            for ($index = $begin; $index <= $end; $index++) { ?>
                                <li class="page-item <?php echo ($index==$page)?'active':false ?>">
                                    <a class="page-link" href="<?php echo _WEB_HOST_ROOT_ADMIN.'?module=blog_categories'.$queryString.'&page='.$index; ?>">
                                        <?php echo $index; ?>
                                    </a>
                                </li>
                            <?php }?>
                            ...
                            if ($page < $maxPage) {
                                $nextPage = $page+1;
                                echo '<li class="page-item">
                <a class="page-link" href="'._WEB_HOST_ROOT_ADMIN.'?module=blog_categories'.$queryString.'&page='.$nextPage.'" 
                    aria-label="Next">
